...
from variation_model_two_strains import SEIR_variation
import pyabc
import os
import uuid
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"09_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"09_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_09_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_09_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    t_alpha_org = datetime(2021, 12, 7)
    # error metric
    ...
    # deaths real
    deaths_real = country_dict["deaths"].loc[(country_dict["deaths"]["year_week"] >= "2020-43") & \
    ...
    history = calibration(SEIR_variation,
                        prior=Distribution(
                                    R0=RV("uniform", 1, 3.3 - 1),
                                    Delta=RV('rv_discrete', values=(np.arange(3, 64), [1. / 61.] * 61)),
                                    start_week_delta=RV('rv_discrete', values=(np.arange(0, 6), [1. / 6.] * 6)),
                                    i0_q = RV("uniform", 60, 120 - 60),  # (20*new_pos-new_pos)
                                    r0_q = RV("uniform", 0.1, 20 - 0.1),
                                    Alpha_increase = RV("uniform", 1.01, 1.5 - 1.01),
...
if __name__ == "__main__":
    ...
    #startdates = (11, 1)
    run_calibration("British Columbia", 9, 14, 7, 4, 12, 19)
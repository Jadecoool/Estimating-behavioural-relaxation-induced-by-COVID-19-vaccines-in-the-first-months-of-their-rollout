...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"new_10_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"new_10_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_new_10_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_new_10_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    t_alpha_org = datetime(2021, 2, 15)
    # error metric
    ...
    history = calibration(SEIR_variation,
                        prior=Distribution(
                                ...
                                i0_q=RV("uniform", 10, 100 - 10),  # (20*new_pos-new_pos)
                                r0_q=RV("uniform", 0.1, 5 - 0.1),
                                alpha=RV("uniform", 0.0001, 1 - 0.0001),
                                gamma=RV("uniform", 0.0001, 1 - 0.0001),
                                r=RV("uniform", 1.01, 1.5 - 1.01),
                                Alpha_increase=RV("uniform", 1.6, 2.5 - 1.6),
...
if __name__ == "__main__":
    ...
    #startdates = (11, 1)
    run_calibration("Sao Paulo", 12, 14, 10, 3, 1, 18)
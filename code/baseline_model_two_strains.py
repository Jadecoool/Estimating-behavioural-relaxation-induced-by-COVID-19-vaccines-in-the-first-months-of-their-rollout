...
def SEIR(start_date_org,
    ...
    # print("intialization I:", compartments[2, age, 0])
    # print("intialization E:", compartments[1, age, 0])
    # print("intialization R:", compartments[3, age, 0])
    # print("intialization S:", compartments[0, age, 0])
    # simulate
    for t in np.arange(1, T, 1):
        ...
        if t >= T_nonvax + 1:
            ...
            # print(compartments[0, age, t - 1])
            for age in range(nage):
                if compartments[0, age, t - 1] < V_S[age]:
                    # print("VS 大于 S个体了，VS加上所有S个体，把S设置为0")
        ...
        # print("疫苗后SV:", compartments[4, :, t - 1])
        # else:
        #     if t >= T_nonvax + 1:
        ...
        total_leaving_from_S = np.random.binomial(compartments[0, :, t - 1].astype(int), 1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha)))
        ...
        total_leaving_from_SV = np.random.binomial(compartments[4, :, t - 1].astype(int), 1 - np.exp(-(prob_SV_to_EV + prob_SV_to_EV_alpha)))
        # print("prob_SV_to_EV", prob_SV_to_EV)
        # print("prob_SV_to_EV_alpha", prob_SV_to_EV_alpha)
        # print("total_leaving_from_SV", total_leaving_from_SV)

...
from baseline_model_two_strains0 import SEIR
import pyabc
import os
import uuid
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"06_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"06_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_06_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_06_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    t_alpha_org = datetime(2021, 1, 4) - timedelta(days=60)
    # error metric
    ...
    history = calibration(SEIR,
                        prior=Distribution(
                                    ...
                                    i0_q=RV("uniform", 1, 150 - 1),  # (20*new_pos-new_pos)
                                    r0_q=RV("uniform", 0.1, 0.4 - 0.1),
                                    #Alpha_increase = RV("uniform", 1.2, 2 - 1.2),
                                    t_alpha_delta = RV('rv_discrete', values=(np.arange(0, 61), [1. / 61.] * 61)) ),
                       params={
                                'end_date': end_date,
                        ...
                        transition = pyabc.AggregatedTransition(
                                            mapping={
                                                'Delta': pyabc.DiscreteJumpTransition(domain=np.arange(7, 36), p_stay=0.7),
                                                ...
                                                't_alpha_delta': pyabc.DiscreteJumpTransition(domain=np.arange(0, 61),
                                                                                      p_stay=0.7),
                                            }
                                        ),
...
if __name__ == "__main__":
    ...
    #startdates = (11, 1)
    # start_simu = 2020-42  start_org=2020-37

...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"08_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"08_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_08_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_08_bas_{filename}.npz",

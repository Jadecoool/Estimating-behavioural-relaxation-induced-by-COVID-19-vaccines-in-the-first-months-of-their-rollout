...
from baseline_model_single_strain2 import SEIR
import pyabc
import os
import uuid
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"vaxS_06_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"vaxS_06_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_vaxS_06_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_vaxS_06_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    history = calibration(SEIR,
                        prior=Distribution(
                                    ...
                                    i0_q=RV("uniform", 5*10**(-4), 10**(-2) - 5*10**(-4)),  # (20*new_pos-new_pos)
...
if __name__ == "__main__":
    ...
    #startdates = (11, 1)
    # start_simu = 2020-47  start_org=2020-42

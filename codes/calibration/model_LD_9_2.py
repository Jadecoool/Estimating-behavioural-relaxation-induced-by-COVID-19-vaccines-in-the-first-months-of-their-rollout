...
def run_fullmodel(start_date_org ,end_date, start_week_delta, vaxstart_date,
                  i0_q, r0_q, R0, alpha, gamma, r, eps, mu, ifr, Delta, VE, VES,
                    Nk, behaviour, google_mobility, basin):

    results = SEIR_variation(start_date_org ,end_date, start_week_delta, vaxstart_date,
                             i0_q, r0_q, R0, alpha, gamma, r, eps, mu, ifr, Delta, VE, VES,
                                Nk, behaviour, google_mobility, basin)
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"vaxS_09_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"vaxS_09_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_vaxS_09_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_vaxS_09_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    # deaths real
    deaths_real = country_dict["deaths"].loc[(country_dict["deaths"]["year_week"] >= "2020-47") & \
...
if __name__ == "__main__":
    ...
    #startdates = (11, 1)
    run_calibration("London", 9, 21, 7, 4, 12, 8)
...
def run_fullmodel(start_date_org ,end_date, start_week_delta, vaxstart_date, t_alpha_org,
    ...
    results = SEIR_behaviour(start_date_org ,end_date, start_week_delta, vaxstart_date, t_alpha_org,
              t_alpha_delta, i0_q, r0_q, R0, eps, mu, alpha, gamma, r, ifr, Delta, VE, VES, VE2, VES2,
              Nk, Alpha_increase, behaviour, google_mobility, basin)
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"new_08_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"new_08_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_new_08_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_new_08_bas_{filename}.npz",
...
def run_calibration(basin, start_month, start_day, end_month, end_day, start_vax_month, start_vax_day):
    ...
    # t_alpha_org = datetime(2020, 8, 17)
    # error metric
    ...
    # deaths real
    deaths_real = country_dict["deaths"].loc[(country_dict["deaths"]["year_week"] >= "2020-41") & \

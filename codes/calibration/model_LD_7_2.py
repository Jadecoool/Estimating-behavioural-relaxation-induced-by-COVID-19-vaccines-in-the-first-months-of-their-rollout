...
def run_fullmodel(start_date_org ,end_date, start_week_delta, vaxstart_date,
    ...
    results = SEIR_behaviour(start_date_org ,end_date, start_week_delta, vaxstart_date,
                             i0_q, r0_q, R0, alpha, gamma, r, eps, mu, ifr, Delta, VE, VES,
                                Nk, behaviour, google_mobility, basin)
...
def calibration(epimodel: Callable,
    ...
    if db == None:
        db_path = os.path.join(f'./calibration_runs/{basin_name}/dbs/', f"vaxS_07_bas_{filename}.db")
    ...
    with open(os.path.join(f'./calibration_runs/{basin_name}/abc_history/', f"vaxS_07_bas_{filename}.pkl"), 'wb') as file:
        pkl.dump(history, file)

    history.get_distribution()[0].to_csv(f"./posteriors/{basin_name}_vaxS_07_bas_{filename}.csv")
    np.savez_compressed(f"./posteriors/{basin_name}_vaxS_07_bas_{filename}.npz",

...
def SEIR_behaviour(start_date_org,
                   end_date,
                   start_week_delta,
                   vaxstart_date,
                   t_alpha_org,
    ...
    if google_mobility==False:
        ...
        beta_Alpha = get_beta(Alpha_increase * R0, mu, Nk, C)
    else:
        # contact matrix
        Cs = {}
        for i in dates:
    ...
    for t in np.arange(1, T, 1):
        ...
        if t >= T_nonvax + 1:
            ...
            if basin=='Lombardy' or basin=='Sao Paulo':
                ...
                V_S, V_S_NC = get_vaccinated(basin, rV_age, compartments[:, :, t - 1].ravel('F'), t, start_date, Nk)
        ...
        total_leaving_to_E = np.random.binomial(total_leaving_from_S, np.array(
            [a / b if b != 0 else 0 for a, b in zip(
                1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha)),
                                                    1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha + prob_S_to_NC)))]))
        # print('force_inf', force_inf)
        # print('force_inf_Alpha', force_inf_Alpha)
        # print('nc_rate', nc_rate)
        # print('1 - np.exp(-force-force_alpha)', 1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha)))
        # print('1 - np.exp(-force-force_alpha)/total', (1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha)))
        #       /(1 - np.exp(-(prob_S_to_E + prob_S_to_E_alpha + prob_S_to_NC))) )
        ...
        # print('------------------------------------------')
        # print('new_E', new_E)
        # print('new_S_NC', new_S_NC)
        # print('new_NC_S', new_NC_S)
        # print('new_E_', new_E_)
        # print('new_E_Alpha_', new_E_Alpha_)
        # print('S', compartments[0, :, t].sum(axis=0))
        # print('SV', compartments[4, :, t].sum(axis=0))
        # print('NC', compartments[14, :, t].sum(axis=0))
        # print('V_NC', compartments[15, :, t].sum(axis=0))
        # print('------------------------------------------')
        ...
        noncompliant[:, t] = compartments[14, :, t] + compartments[15, :, t]
        ...
        if t_rv < "2021-01-03":
            ...
            for age in range(nage):
                ...
                if y[(ncomp * age) + 0] <= 0:
                    ...
                    V_S[age] = round(
                        rV1 * y[(ncomp * age) + 0] / den_age * Nk[age] * Nk[age] / (Nk[14] + Nk[15]) + rV2 * Nk[age])
                    V_S_NC[age] = round(rV1 * y[(ncomp * age) + 14] / den_age * Nk[age] * Nk[age] / (Nk[14] + Nk[15]) + rV2 * Nk[age])  # 75 + years

...
def run_cf(basin):
    ...
    if basin == 'London':
        from baseline_model_single_strain_novaccine import SEIR
    else:
        from baseline_model_two_strains_novaccine import SEIR
        from baseline_model_two_strains_cf_mobility import SEIR_cf_mobility
    ...
    for idx, data in pos.iterrows():
        ...
        if basin == 'London':
            ...
            death_sim_vax, infection_sim_vax = SEIR(start_date_org=start_date_org, end_date=end_date, start_week_delta=int(t0),
            ...
            death_sim_novax, infection_sim_novax = SEIR_cf_mobility(start_date_org=start_date_org, end_date=end_date, start_week_delta=int(t0),
                                          vaxstart_date=vaxstart_date,
                                          i0_q=i0_q, r0_q=r0_q, R0=R0,
                                          eps=eps, mu=mu, ifr=IFR, Delta=Delta,
                                          VE=VE, VES=VES,
            ...
            death_sim_vax, infection_sim_vax = SEIR(start_date_org=start_date_org, end_date=end_date,
            ...
            death_sim_novax, infection_sim_novax = SEIR_cf_mobility(start_date_org=start_date_org,
    ...
    np.savez_compressed(f"./posteriors/counterfactual_remove_mobility/death_new(different lengths of trajectories)/{basin}_death_yesmob.npz", death_samples_vax)
    np.savez_compressed(f"./posteriors/counterfactual_remove_mobility/infection_new(different lengths of trajectories)/{basin}_infection_yesmob.npz", infection_samples_vax)

    np.savez_compressed(f"./posteriors/counterfactual_remove_mobility/death_new(different lengths of trajectories)/{basin}_death_nomob.npz", death_samples_novax)
    np.savez_compressed(f"./posteriors/counterfactual_remove_mobility/infection_new(different lengths of trajectories)/{basin}_infection_nomob.npz", infection_samples_novax)